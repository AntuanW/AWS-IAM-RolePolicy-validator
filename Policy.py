...
class Policy:
    ...
    def __validate_json(self, json_object) -> None:
        ...
        def validate_policy_document(policy_document) -> None:
            if not isinstance(json_object["PolicyDocument"], dict):
                raise ValueError("PolicyDocument must be a dictionary")
            if json_object["PolicyDocument"].keys() != {"Version", "Statement"}:
                raise KeyError(
                    "Invalid PolisyDocument inside JSON, check the JSON structure."
                )
            if not isinstance(json_object["PolicyDocument"]["Statement"], list):
                raise ValueError("Statement must be a list")

            for statement in json_object["PolicyDocument"]["Statement"]:
                if not isinstance(statement, dict):
                    raise ValueError("Statement must be a dictionary")

        if json_object.keys() != {"PolicyName", "PolicyDocument"}:
            raise KeyError("Invalid input JSON key, check the JSON structure.")
